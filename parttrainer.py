...
class Trainer:
    ...
    def train(self, epoch_id):
        ...
        for batch, inputs in enumerate(zip(*self.train_set)):
            ...
            p_dist = self.distance(a1, p1) + self.distance(a2, p2) + self.distance(a3, p3)
            n_dist = self.distance(a1, n1) + self.distance(a2, n2) + self.distance(a3, n3)
            correct = (p_dist < n_dist).sum().item()
